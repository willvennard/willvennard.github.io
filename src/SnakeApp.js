...
function directionHelper(direction) {
  if (snakeGame.snakeDirection === "ArrowLeft" && direction === "ArrowRight") {
    return snakeGame.snakeDirection;
  }
  if (snakeGame.snakeDirection === "ArrowRight" && direction === "ArrowLeft") {
    return snakeGame.snakeDirection;
  }
  if (snakeGame.snakeDirection === "ArrowUp" && direction === "ArrowDown") {
    return snakeGame.snakeDirection;
  }
  if (snakeGame.snakeDirection === "ArrowDown" && direction === "ArrowUp") {
    return snakeGame.snakeDirection;
  }
  return direction;
}
...
class SnakeGame {
  ...
  randomBoardCord() {
    ...
    let x =
      Math.round((Math.random() * this.boardWidth) / segDim) * segDim - segDim;
    let y =
      Math.round((Math.random() * this.boardHeight) / segDim) * segDim - segDim;
    // keep food on map
  ...
  gameTick() {
    ...
    if (this.foodCord.length > 0) {
      if (
        this.foodCord[0] === this.headCord[0] &&
        this.foodCord[1] === this.headCord[1]
      ) {
    ...
    // losing conditions
    if (
      this.headCord[0] >= this.boardWidth ||
      this.headCord[0] < 0 || // off the sides
      this.headCord[1] < 0 ||
      this.headCord[1] >= this.boardHeight
    ) {
...
class SnakeBombs {
  ...
  spawnBomb() {
    ...
    d3.select("#board")
      ...
      .attr(
        "transform",
        "translate(" +
          (this.bombCord[0] + this.snakeGame.segmentDim / 4) +
          "," +
          (this.bombCord[1] + this.snakeGame.segmentDim - 1) +
          ")"
      );
  ...
  bombGoesBoom() {
    ...
    for (let block of explosionArea) {
      ...
      let distance =
        distanceBetweenPoints(this.bombCord, block) / this.snakeGame.segmentDim;
  ...
  isHeadInExplosion() {
    ...
    let explosionXRange = [
      this.bombCord[0] - segDim * bombR,
      this.bombCord[0] + segDim * bombR,
    ];
    let explosionYRange = [
      this.bombCord[1] - segDim * bombR,
      this.bombCord[1] + segDim * bombR,
    ];
    let detectX =
      this.snakeGame.headCord[0] >= explosionXRange[0] &&
      this.snakeGame.headCord[0] <= explosionXRange[1];
    let detectY =
      this.snakeGame.headCord[1] >= explosionYRange[0] &&
      this.snakeGame.headCord[1] <= explosionYRange[1];
    if (detectX && detectY) {
